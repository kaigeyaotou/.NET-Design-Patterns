    ...
    public class UpdateValidator : AbstractValidator<Update.Command>
    {
        ...
        private void Validate(Update.Command message, CustomContext context)
        {
            using (var scope = _databaseScopeFactory.CreateReadOnly())
            {
                ...
                var nameValidate = _materialRepository.ExistNameAsync(message.MateName, message.MateTypeId, message.Id).GetAwaiter().GetResult();
                if (nameValidate.Any())
                {
                    context.AddBadRequestError("物料名称不能重复");
                }

                var list = _materialRepository.BasicExistAsync(message, message.Id).GetAwaiter().GetResult().ToList();
                if (list.Any())
                {
